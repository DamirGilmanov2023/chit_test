...
while rep<int(repeat):
	...
	while step1:
		...
		avar=vopros_content.find_elements(By.CLASS_NAME,"dfdTest_spanAVariants")
		avar_text=""
		if len(avar)!=0:
			table=vopros_content.find_elements(By.TAG_NAME,"table")[0]
			label=table.find_elements(By.TAG_NAME,"label")
			input_type="avar"
			if len(label)==len(avar):
				i=0
				r=[]
				while i<len(label):
					m=random.randint(0,len(avar)-1)
					if m not in r:
						r.append(m)
						n=0
						while n<2:
							driver.execute_script('arguments[0].scrollIntoView(true);', avar[m])
							time.sleep(1)
							avar[m].click()
							time.sleep(1)
							driver.execute_script('arguments[0].scrollIntoView(true);', label[i])
							time.sleep(1)
							label[i].click()
							time.sleep(1)
							n+=1
						try:
							avar_img=avar[m].find_elements(By.TAG_NAME,"img")[0]
							avar_attr_img=avar_img.get_attribute("src")
							avar_spl_attr_img=avar_attr_img.split("/")
							avar_name_img=avar_spl_attr_img[len(avar_spl_attr_img)-1]
							urllib.request.urlretrieve(avar_attr_img,f"./img/{avar_name_img}")
							a_t=avar[m].text+f"<img src='./img/{avar_name_img}'/>"
						except:
							a_t=avar[m].text

						try:
							label_img=label[i].find_elements(By.TAG_NAME,"img")[0]
							label_attr_img=label_img.get_attribute("src")
							label_spl_attr_img=label_attr_img.split("/")
							label_name_img=label_spl_attr_img[len(label_spl_attr_img)-1]
							urllib.request.urlretrieve(label_attr_img,f"./img/{label_name_img}")
							l_t=label[i].text+f"<img src='./img/{label_name_img}'/>"
						except:
							l_t=label[i].text
				...
				while i<len(label):
					...
					time.sleep(2)
					if i!=len(label)-1:
						avar_text+=label[i].text+"---"+avar[m].text+":::"
					else:
						avar_text+=label[i].text+"---"+avar[m].text
					i+=1
		if input_type!="avar":
		...
		if input_type!="avar":
			...
			for lbl in label:
				try:
					label_img=lbl.find_elements(By.TAG_NAME,"img")[0]
					label_attr_img=label_img.get_attribute("src")
					label_spl_attr_img=label_attr_img.split("/")
					label_name_img=label_spl_attr_img[len(label_spl_attr_img)-1]
					urllib.request.urlretrieve(label_attr_img,f"./img/{label_name_img}")
					lll=lbl.text+f"<img src='./img/{label_name_img}'/>"
				except:
					lll=lbl.text
				label_mass.append(lll)
			for tru in true:
	...
	if input_type=="radio" or input_type=="checkbox":
		...
		while i<len(check_mass):
			...
			i+=1
		vrm_mass.append(v)
	elif input_type=="avar":
		vrm_mass.append(avar_text)
	if input_type!="avar":
		for lbl in label_mass:
			vrm_mass.append(lbl)
	#print(vrm_mass)
